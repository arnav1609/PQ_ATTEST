...
//
//-----------------------------------------------------------------------------
// 7c INTERFACE CONTRACT (frozen 2026-09-23, docs/decisions/7c-flow-control.md)
//-----------------------------------------------------------------------------
// Mesh ports (N/S/E/W): credit-based, per VC.
//   credit_return[p][v] : downstream freed one slot of VC v behind output p.
//   credit_out[p][v]    : this router popped one flit from its own input FIFO
//                         p/VC v this cycle (return it to the upstream router).
// LOCAL port: ready/valid, both directions.
//   in  : transfer = in_valid_local  && in_ready_local.
//         in_ready_local = !full of the LOCAL input FIFO named by
//         in_flit_local.vc_id (conservative: no same-cycle pop credit).
//   out : transfer = out_valid_local && out_ready_local.
//
// LOCAL EJECT BUFFER. The NI's noc_rx_ready is combinational on noc_rx_valid
// and noc_rx_flit, and the allocator's round-robin pointer advances on a raw
// grant. Driving out_valid_local straight from the crossbar would therefore
// (a) close a combinational loop if ready gated valid, and (b) let the LOCAL
// flit change while valid && !ready. Both are removed by keeping the verified
// credit protocol on LOCAL *inside* the router and terminating it in a FIFO:
//   - credits for LOCAL are generated internally from eject-FIFO pops;
//     credit_return[PORT_LOCAL] is IGNORED;
//   - out_valid_local = !eject_empty  (register-derived, independent of ready);
//   - out_flit_local  = eject head    (stable until popped);
//   - eject depth = NUM_VC * VC_DEPTH = 12 = total LOCAL credit, so the eject
//     FIFO can never overflow (its own overflow SVA proves it in sim).
// Cost: +1 cycle LOCAL eject latency, one 12-entry flit FIFO per router.
//=============================================================================

module noc_router #(
    // EJECT_PIPE (D7b timing fix, 2026-09-24; default 1 = synthesized config).
    //   1 : the crossbar LOCAL output and its write strobe are registered once
    //       before the 12-entry eject FIFO (eject_wr_q / eject_flit_q).
    //   0 : crossbar writes the eject FIFO in the allocation cycle (pre-D7b).
    // Why: N-2.5 run 2 (LINK_PIPE=1) failed WNS -0.317 ns; the worst path was
    // input-FIFO head -> allocator -> crossbar -> eject-FIFO LUTRAM WE inside
    // ONE router (20 levels, WE fanout 52). Run-1 triage: all 132 failing
    // intra-router endpoints were eject-FIFO LUTRAM pins.
    // Safety: the LOCAL credit is consumed at grant (M9), so a flit in the
    // stage already owns an eject slot; eject depth 12 = LOCAL credit 12, so
    // no overflow. out_valid_local stays = !eject_empty (register-derived).
    // Cost: +1 cycle LOCAL delivery latency, 37 FFs per router.
    parameter bit EJECT_PIPE = 1'b1
) (
    ...
    output logic            in_ready_local,     // 7c: LOCAL input ready

    // One-cycle credit return per downstream output / VC.
    // [PORT_LOCAL] is ignored: LOCAL credit is generated internally (7c).
    input logic [noc_pkg::NUM_VC-1:0] credit_return [noc_pkg::NUM_PORTS],

    // One-cycle credit to the UPSTREAM router: own input FIFO p/VC v popped.
    // [PORT_LOCAL] is tied to 0: the LOCAL input uses in_ready_local (7c).
    output logic [noc_pkg::NUM_VC-1:0] credit_out [noc_pkg::NUM_PORTS],
    ...
    input  logic            out_ready_local     // 7c: NI accepts LOCAL flit
);
    ...
    // 7c: LOCAL input gating and LOCAL eject buffer.
    logic  in_valid_local_acc;             // accepted LOCAL input
    logic  eject_wr, eject_rd, eject_empty, eject_full;
    flit_t eject_head;
    logic [NUM_VC-1:0] credit_return_int [NUM_PORTS];

    localparam int unsigned EJECT_DEPTH = VC0_DEPTH + VC1_DEPTH + VC2_DEPTH;
    ...
    noc_router_datapath u_datapath (
        ...
        .in_flit_local (in_flit_local), .in_valid_local (in_valid_local_acc),
    ...
    noc_credit_control u_credit_control (
        ...
        .grant_valid           (grant_valid),
        .xbar_sel              (xbar_sel),
        .credit_return         (credit_return_int),
        .credit_ok             (credit_ok),
    ...
    //=========================================================================
    // 7c LOCAL INPUT : ready/valid
    //=========================================================================

    always_comb begin
        // Illegal VC (>= NUM_VC): accept so the NI cannot hang; M4 discards
        // it and its own illegal-VC handling reports it (unchanged behaviour).
        if      (int'(in_flit_local.vc_id) == 0) in_ready_local = !full_local_vc0;
        else if (int'(in_flit_local.vc_id) == 1) in_ready_local = !full_local_vc1;
        else if (int'(in_flit_local.vc_id) == 2) in_ready_local = !full_local_vc2;
        else                                     in_ready_local = 1'b1;
    end

    assign in_valid_local_acc = in_valid_local & in_ready_local;

    //=========================================================================
    // 7c LOCAL OUTPUT : credit -> eject FIFO -> ready/valid
    //=========================================================================

    assign eject_wr = xb_val_local & grant_valid_qualified[PORT_LOCAL];
    assign eject_rd = out_valid_local & out_ready_local;

    // D7b eject stage (see EJECT_PIPE in the header).
    flit_t eject_flit_q;
    logic  eject_wr_q;
    generate
        if (EJECT_PIPE) begin : G_EJECT_REG
            always_ff @(posedge clk) begin
                if (rst) begin
                    ...
                    eject_wr_q   <= eject_wr;
                    eject_flit_q <= xb_out_local;
                end
            end
        end
        else begin : G_EJECT_WIRE
            assign eject_wr_q   = eject_wr;
            assign eject_flit_q = xb_out_local;
        end
    endgenerate

    noc_fifo #(.DEPTH(EJECT_DEPTH)) u_eject_fifo (
        .clk        (clk),
        .rst        (rst),
        .wr_flit    (eject_flit_q),
        .wr_en      (eject_wr_q),
        .rd_en      (eject_rd),
        .rd_flit    (eject_head),
        .empty      (eject_empty),
    ...
    assign out_valid_local = !eject_empty;
    assign out_flit_local  = eject_head;

    // Credit into M9: mesh ports from outside, LOCAL from eject pops.
    always_comb begin
        for (int p = 0; p < NUM_PORTS; p++) begin
            credit_return_int[p] = credit_return[p];
        end
        credit_return_int[PORT_LOCAL] = '0;
        if (eject_rd && (int'(eject_head.vc_id) < NUM_VC)) begin
            credit_return_int[PORT_LOCAL][eject_head.vc_id] = 1'b1;
        end
    end

    //=========================================================================
    // 7c CREDIT OUT : own input FIFO pops, to the upstream routers
    //=========================================================================

    assign credit_out[PORT_NORTH] = {credit_north_vc2, credit_north_vc1, credit_north_vc0};
    assign credit_out[PORT_SOUTH] = {credit_south_vc2, credit_south_vc1, credit_south_vc0};
    assign credit_out[PORT_EAST ] = {credit_east_vc2,  credit_east_vc1,  credit_east_vc0 };
    assign credit_out[PORT_WEST ] = {credit_west_vc2,  credit_west_vc1,  credit_west_vc0 };
    assign credit_out[PORT_LOCAL] = '0;
    ...
    // M-F1 INVARIANT I-3 (2026-09-24): the eject FIFO must hold every LOCAL
    // credit the M9 counters can hand out (sum of per-VC depths), or a flit
    // granted against a valid credit can find the eject FIFO full. EJECT_PIPE
    // relies on this: the flit in eject_flit_q already owns an eject slot.
    // Guards a future edit of EJECT_DEPTH or of one VCx_DEPTH but not the other.
    initial begin
        if (EJECT_DEPTH < (VC0_DEPTH + VC1_DEPTH + VC2_DEPTH))
            $fatal(1, "noc_router: EJECT_DEPTH %0d < total LOCAL credit %0d (M-F1 I-3)",
                   EJECT_DEPTH, VC0_DEPTH + VC1_DEPTH + VC2_DEPTH);
    end
    ...
    // LOCAL is buffered (7c): the grant check moves to the eject write.
    a_eject_wr_implies_grant_l: assert property (@(posedge clk) disable iff (rst)
        eject_wr |-> grant_valid[PORT_LOCAL])
        else $error("noc_router: LOCAL eject write without a LOCAL grant");

    // 7c ready/valid rules on the LOCAL output.
    a_local_out_valid_hold: assert property (@(posedge clk) disable iff (rst)
        (out_valid_local && !out_ready_local) |=> out_valid_local)
        else $error("noc_router: LOCAL out_valid dropped before acceptance");
    a_local_out_flit_hold: assert property (@(posedge clk) disable iff (rst)
        (out_valid_local && !out_ready_local) |=> $stable(out_flit_local))
        else $error("noc_router: LOCAL out_flit changed before acceptance");

    // Eject FIFO sized to total LOCAL credit: a write while full means the
    // credit accounting is broken.
    a_eject_no_overflow: assert property (@(posedge clk) disable iff (rst)
        !(eject_wr_q && eject_full))
        else $error("noc_router: LOCAL eject FIFO overflow - credit accounting broken");

    // (LOCAL input overflow is checked by M4's own FIFO overflow SVA: with
    //  in_ready_local gating, any LOCAL FIFO overflow now means a real bug.)
