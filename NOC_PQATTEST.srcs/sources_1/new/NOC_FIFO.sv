...
module noc_fifo #(
    ...
    // ------------------------------------------------------------------------
    // FIX 2026-09-23: was (ptr < PTR_WIDTH'(DEPTH)). For a power-of-two DEPTH
    // the size cast truncates DEPTH to 0 (IEEE 1800 6.24.1: 2'(4) == 2'b00),
    // so the property was 'ptr < 0' - always false. Verilator fired it on the
    // first unreset edge; XSim reported no firing (mechanism not established).
    // Compared as int now: a tautology for power-of-two DEPTH, a real check for
    // the 12-deep eject FIFO.

    property p_write_pointer_in_range;
        @(posedge clk) disable iff (rst)
        (int'(wr_ptr) < int'(DEPTH));
    endproperty
